...
- **修复:`CONFIG_TTL_SECONDS=0` 现在可让保存的配置永久存储(永不过期)。** Node 运行时此前用 `||` 把 `0` 转成了 `undefined`,于是静默回落到 30 天默认值,导致无法通过配置实现永久存储。现改用 `??`,让 `0` 穿透到 KV 层,写入不带过期时间的键。未设置时仍默认 30 天。

### v2.11.2

- **修复:与自定义规则 / 规则集同名的自定义策略组不再被覆盖。** 当某条规则路由到同名组(例如经 `DOMAIN-SUFFIX,…,Ponte MacMini` 指向 Surge 的 `DEVICE:` 设备组)时,自动生成的规则组会先占用该名字,用户自己的组被当作重复项丢弃,转而输出全部节点列表。现在显式定义的组在 Clash / sing-box / Surge 上都优先占用该名字;成员解析按平台区分,只含设备的组在 Surge 上得以保留,在 Clash/sing-box 上则正确为空(规则回退到节点选择器)。
- **调整:自定义策略组不再被自动列入其它策略组的成员里。** 此前它们会出现在「节点选择」及每个规则组的可选项里;现在独立存在 —— 仍可作为「自定义规则 / 自定义规则集」的路由目标,以及在被显式选为「漏网之鱼」/ 规则集默认出站时出现在那一个组里(select 组的默认项必须是其成员)。
